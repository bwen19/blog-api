...
const publishPost = `-- name: PublishPost :many
UPDATE posts SET status = 'publish'
WHERE id = ANY($1::bigint[]) AND status = 'review'
RETURNING id, author_id
`

type PublishPostRow struct {
	ID       int64 `json:"id"`
	AuthorID int64 `json:"author_id"`
}

func (q *Queries) PublishPost(ctx context.Context, ids []int64) ([]PublishPostRow, error) {
	rows, err := q.db.Query(ctx, publishPost, ids)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []PublishPostRow{}
	for rows.Next() {
		var i PublishPostRow
...
const submitPost = `-- name: SubmitPost :many
...
func (q *Queries) SubmitPost(ctx context.Context, arg SubmitPostParams) ([]int64, error) {
	rows, err := q.db.Query(ctx, submitPost, arg.Ids, arg.AuthorID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
SET
  ...
  update_at = now()
WHERE id = $1 AND author_id = $6::bigint
  AND status = ANY('{draft, revise}'::varchar[])
RETURNING id, author_id, title, abstract, cover_image, content, status, is_featured, view_count, update_at, publish_at
`
...
const withdrawPost = `-- name: WithdrawPost :many
...
type WithdrawPostRow struct {
	ID       int64 `json:"id"`
	AuthorID int64 `json:"author_id"`
}

func (q *Queries) WithdrawPost(ctx context.Context, ids []int64) ([]WithdrawPostRow, error) {
	rows, err := q.db.Query(ctx, withdrawPost, ids)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []WithdrawPostRow{}
	for rows.Next() {
		var i WithdrawPostRow
		if err := rows.Scan(&i.ID, &i.AuthorID); err != nil {
			return nil, err
		}
		items = append(items, i)
